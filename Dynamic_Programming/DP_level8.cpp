...
/*In this problem we have to caluclate distinct palindromic subsequnces in the string.
In the above problem we caluclated all the palindormic subsequences irresepctive of whether the strings are unique or not.
For that let us reprsent a string s for which we want to caluclate the distinct palindromic subsequences.

==> s can also be represented as c1mc2 which c1+middle string+c2 where c1 and c2 are the first and last charactes of
the string respectively.

==>Let us represnt the CdPs(s) as function which gives us the set distinct palindromic subsequences
==>Let us represent s1=CdPs(m),s2=CdPs(c1m),s3=CdPs(mc2),s4=CdPs(c1mc2) as the set of distinct palindromic subsequences.
**Note that last s4 is not whole string it is that c1 and c2 must be there in those strings and middle part can be anything.
This will not be equal to whole strings.

==>CdPs(s)=s1+s2+s3+s4-s1s2-s1s3-s1s4-s2s3-s2s4-s3s4+s1s2s3+s1s3s4+s2s34s4-s1s2s3s4

**It is the basic inclusion-exclusion principle
Upon the above condition there will be the following cases upon simplificatiion in generating all the substrings and cancelling in b/w
them


1.c1!=c2  ----------------------------------------------------------- dp[i][j]=dp[i+1][j]+dp[i][j-1]-dp[i+1][j-1];

                                  no c1 or c2 char present in b/w
                               ------------------------------------ dp[i][j]=2*dp[i+1][j-1]+2
                              |
                              |
                              |
                              | 1 c1 is present in b/w (i.e,. in m)
2.c1==c2----------------------|------------------------------------- dp[i][j]=2*dp[i+1][j-1]+1
                              |
                              |
                              |
                              |
                              |
                               -------------------------------------- dp[i][j]=2*dp[i+1][j-1]-dp[nc+1][pc-1] where nc and pc are the very 
                                                                                                             next index of c1 and very previous
                                                                                                             index of c2.
                                                                                        

For proving some condition on above we need more knowledge on sets.
we can simply take a string with the above condition and validate above functions.
*/
ll countDistnticPalindromicSubsequences(string &s){
     ll n = s.size();
        unordered_map<char, int> mp;
        vector<ll> prev(n, 0), next(n, 0);
            ...
            for (int i = 0, j = g; j < n;i++,j++){
                if(g==0)
                    ...
                    if(s[i]!=s[j]){
                        ...
                        int nc = next[i], pc = prev[j];
                        if(nc>pc)
                            dp[i][j] =((dp[i+1][j-1] + dp[i + 1][j - 1])%mod + 2) % mod;
                        else if(nc==pc)
                            dp[i][j] = ((dp[i+1][j-1] + dp[i + 1][j - 1])%mod + 1) % mod;
                        else 
                            dp[i][j] = ((dp[i+1][j-1] + dp[i + 1][j - 1])%mod - dp[nc + 1][pc - 1]+mod) % mod;
                    }
                }
            }
        }
