...
/*undirected graph*/
bool cycle_dfs(int node,int parent,vector<vector<int>> &graph){
    vis[node]=true;
    for(auto &x:graph[node]){
        if(x==parent) continue;
        else if(vis[x]) return true;
        else if(cycle_dfs(x,node,graph)) return true;
    }
    return false;
}
...
If the graph is possible to divide the vertices into two exclusive and exhaustive sets then graph is bipartite.
i.Acyclic graph is always bipartite
ii.Cyclic graph with even size edges is bipartite but not odd size edges*/
