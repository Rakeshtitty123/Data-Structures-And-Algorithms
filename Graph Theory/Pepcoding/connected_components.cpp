...
ll find(ll x,vil &parent){
    if(parent[x]==x)
        return x;
    return parent[x] = find(parent[x], parent);
}
bool _union(ll x,ll y,vil &parent, vil &size){
...
void solve(){
    ...
    ll total = 0;
    vil edgeWeight(maxW+1, 0);
    for (ll i = 0; i < n-1;i++){
        auto x = edges[i];
        ll px = find(x.second.first,parent), py = find(x.second.second,parent);
        if(px!=py)
            edgeWeight[x.first] += (size[px] * size[py]);
        _union(x.second.first, x.second.second, parent, size);
    }
    for (ll i = 1; i <= maxW;i++)
