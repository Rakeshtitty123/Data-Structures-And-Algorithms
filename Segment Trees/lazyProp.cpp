...
void update(ll node,ll start,ll end,ll left,ll right,ll val,vil &segTree,vil &v){
    if(end<left or right<start)
        ...
        ll mid = (start + end) / 2;
        update(node * 2, start, mid, left, right, val, segTree, v);
        update(node * 2 + 1, mid + 1, end, left, right, val, segTree, v);
    }
}
ll query(ll node,ll start,ll end,ll idx,vil &segTree){
    ...
    if (start == end){
        ...
        //propagate cur stored value to the node
         segTree[node * 2] += segTree[node];
        segTree[node * 2 + 1] += segTree[node];
         segTree[node] = 0;
        ll mid = (start + end) / 2;
        if(idx>=start and idx<=mid){
            return query(node * 2, start, mid, idx, segTree);
        }else
            return query(node * 2 + 1, mid + 1, end, idx, segTree);
    }
}
void solve(){
        ll n;
        ...
        buildTree(1, 0,n - 1,segTree, v);
         debug(segTree);
     //   update(1, 0, n - 1, 1, 2, 6, segTree, v);
        debug(segTree);
       // cout << segTree[5] << segTree[6];
       // cout << segTree[9] << '\n';
       // cout << query(1, 0, n - 1, 1, segTree);
        debug(segTree);
        ll q;
        cin >> q;
        while(q--){
            ...
            if(a==1){
                ...
                cin >> left >> right >> val;
               // cout << a;
                update(1, 0, n - 1, left, right, val, segTree, v);
            }else {
                ll idx;
                cin >> idx;
                cout << query(1, 0, n - 1, idx, segTree)<<'\n';
            }
        }
}
int main(){
