...
void update(ll node,ll start,ll end,ll left,ll right,vil &segTree,vil &lazy){
    propagate(node, start, end, segTree, lazy);
    if(end<left or right<start)
        return;
    else if(start==end)
        ...
        ll mid = (start + end) / 2;
        update(node * 2, start, mid, left, right, segTree,lazy);
        update(node * 2 + 1, mid + 1, end, left, right, segTree,lazy);
...
void solve(){
        ...
        while(q--){
               ...
               if(qtype==1){
                   ...
                   //left--;
                   //right--;
                   update(1, 1,n, left, right, segTree,lazy);
               }else{
                   ll left, right;
                   cin >> left >> right;
                  // left--;
                  //r ight--;
                   cout << query(1, 1,n ,left, right, segTree,lazy) << '\n';
               }
        }
}
int main(){
